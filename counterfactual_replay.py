...
def prepare_historical_technicals(raw, direction, entry_price):
...
def trailing_stop_after_high(entry, target, current_stop, highest):
    target_gap = target - entry
    if target_gap <= 0:
        return current_stop

    progress = (highest - entry) / target_gap
    new_stop = current_stop
    if progress >= 0.25:
        new_stop = max(new_stop, round(entry * 0.99, 0))
    if progress >= 0.40:
        new_stop = max(new_stop, round(entry, 0))
    if progress >= 0.60:
        new_stop = max(new_stop, round(entry + target_gap * 0.30, 0))
    if progress >= 0.75:
        new_stop = max(new_stop, round(entry + target_gap * 0.50, 0))
    if progress >= 0.90:
        new_stop = max(new_stop, round(entry + target_gap * 0.70, 0))
    return new_stop


def simulate_trade(candles, signal_time, entry, target, stop, quantity):
    ...
    for timestamp, candle in future.iterrows():
        ...
        if low <= current_stop:
            reason = "STOP_AND_TARGET_SAME_CANDLE" if high >= target else "STOP_LOSS"
            exit_price = current_stop
            return {
                "exit_time": timestamp.isoformat(),
                "exit_price": round(exit_price, 2),
                "exit_reason": reason,
                "gross_pnl": round((exit_price - entry) * quantity, 2),
        ...
        if high >= target:
            return {
                ...
                "exit_reason": "TARGET",
                "gross_pnl": round((target - entry) * quantity, 2),
                "highest_price": round(highest, 2),
                "lowest_price": round(lowest, 2),
            }

        current_stop = trailing_stop_after_high(
            entry,
            target,
            current_stop,
            highest,
...
def grade_candidate(symbol, raw):
    ...
    technicals = prepare_historical_technicals(raw, direction, entry)
    ...
    target, stop = option_levels_from_fill(entry, target_percent, stop_percent)
...
def replay_day(date_text):
    ...
    for _, journal_row in analysis.iterrows():
        ...
        if open_until[symbol] is not None and signal_time <= open_until[symbol]:
            decisions.append({**base, "decision": "SKIP", "reason": "Existing simulated position still open"})
            continue
        ...
        quantity = order_quantity_for(symbol, instrument, entry, stop)
        if quantity <= 0:
            decisions.append(
                {
                    **base,
        ...
        outcome = simulate_trade(candles, signal_time, entry, target, stop, quantity)
