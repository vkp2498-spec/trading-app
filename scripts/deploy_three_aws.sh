...
echo "Phase 3/3: applying knowledge-engine roles and restarting services"
for index in "${!hosts[@]}"; do
  host="${hosts[$index]}"
  role="${roles[$index]}"
  echo "[$host] applying role: $role"
  reset_command=""
  if [[ "$reset_tracking_data" == true && "$role" == "kb-shadow" ]]; then
    reset_command="venv/bin/python reset_tracking_data.py --confirm && "
  fi
  if [[ "$role" == "kb-shadow" || "$role" == "kb-live-max" || "$role" == "kb-live-one-lot" ]]; then
    role_command="rm -f .trading_cron_disabled && venv/bin/python scripts/sync_trading_cron.py --app-dir $remote_dir_quoted"
  else
