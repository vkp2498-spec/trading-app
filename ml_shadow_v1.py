...
def add_forward_labels(features: pd.DataFrame, horizon_candles: int, minimum_move: float) -> pd.DataFrame:
    ...
    labeled["label"] = "NONE"
    call_mask = (upward >= minimum_move) & (upward > downward)
    put_mask = (downward >= minimum_move) & (downward > upward)
    labeled.loc[call_mask, "label"] = "CALL"
    labeled.loc[put_mask, "label"] = "PUT"
...
def train() -> dict:
    ...
    if set(class_counts) != {"CALL", "PUT", "NONE"}:
        raise RuntimeError(f"All CALL/PUT/NONE classes are required: {class_counts}")
    ...
    qualified_mask = (
        ...
        & (validation_confidence >= shadow_threshold)
    )
