...
hosts=("$1" "$2" "$3")
roles=("ml-live" "disabled" "disabled")
...
for index in "${!hosts[@]}"; do
  ...
  if [[ "$role" == "ml-shadow" || "$role" == "ml-live" ]]; then
    role_command="rm -f .trading_cron_disabled && venv/bin/python ml_shadow_4h_v2_live.py --train && venv/bin/python ml_shadow_v1.py --train && venv/bin/python scripts/sync_trading_cron.py --app-dir $remote_dir_quoted"
  else
...
echo "Deployment complete: Vamsi=V2 one-lot live plus V3 forecast-only; Ganesh/Sastry disabled."
